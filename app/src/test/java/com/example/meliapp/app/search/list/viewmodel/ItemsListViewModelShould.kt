...
class ItemsListViewModelShould : BaseUnitTest() {
    ...
    private val searchItemsByQuery: SearchItemsByQuery = mock()
    private val itemsResponse: ItemsResponse = mock()

    private val expected = Result.success(itemsResponse)
    private val exception = RuntimeException("Something went wrong")

    @Test
    fun `when searchItemsByQuery is executed then the searchItemsByQuery action is executed`() =
        runBlockingTest {
    ...
    @Test
    fun `when searchItemsByQuery is executed then the searchItemsByQuery action emits itemResponse`() =
        runBlockingTest {
            val viewModel = mockSuccessfulCase()
